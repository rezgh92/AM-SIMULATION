...
def objective(x, budget_h, kind=None):
...
if __name__ == "__main__":
    ...
    def cb(xk, convergence=None):
        f = objective(xk, budget, OBJECTIVE)
        hist.append(dict(t=time.time() - t0, f=f, x=list(map(float, xk))))
        print(f"[{time.time()-t0:7.0f}s] f={f:.4f} conv={convergence}", flush=True)

    res = differential_evolution(objective, BOUNDS, args=(budget, OBJECTIVE), maxiter=maxiter, popsize=popsize, tol=1e-3,
    ...
    json.dump(dict(budget_h=budget, objective=OBJECTIVE, x=list(map(float, res.x)), names=NAMES, f=float(res.fun), metrics=m,
