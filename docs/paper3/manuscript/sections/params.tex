...
No measurements were made for this study. Every input therefore comes from one of five sources, and Table~\ref{tab:params} states which one applies. A value is labelled L when it was reported for the glass-ceramic of the IBM process or for copper itself; A when it was taken from an analogous material, usually a commercial copper-compatible LTCC; C when it was calibrated here against published process observations (Section~\ref{sec:calibration}); D when it follows from other entries of the table; and E when it is an engineering estimate. Estimated and analogue values that matter enter the sensitivity analysis of Section~\ref{sec:res_sens}, with the ranges given in the table.

The reference glass is the MgO-rich cordierite-type glass used by IBM for its copper-compatible substrates (glass \#12: 52.5 SiO$_2$, 22.0 MgO, 22.0 Al$_2$O$_3$, 1.5 P$_2$O$_5$, 0.5 B$_2$O$_3$, 1.5 ZrO$_2$, wt\%) \cite{herron1980method,kumar1981glassceramic}. It was chosen because it is the only copper-compatible glass-ceramic for which the firing process, the atmosphere and the reasons behind the chosen temperatures have been published in detail. Its viscosity has not been published in a form we could read, so it was calibrated.
...
Median particle size & $d_{50}$ & 3.0~$\mu$m & A & IBM glass powders 2--7~$\mu$m \cite{kumar1981glassceramic}; 2--7~$\mu$m \\
...
Density at pore closure & $\rho_{\mathrm{cl}}$ & 0.92 & E & \\
...
Poisson's ratio & $\nu_{\mathrm{GC}}$ & 0.25 & E & \\
Thermal expansion coefficient & $\alpha_{\mathrm{GC}}$ & 3.0~ppm\,K$^{-1}$ & L & production glass-ceramic substrate \cite{knickerbocker2002advanced} \\
...
Median particle size & $d_{50}$ & 1--20~$\mu$m & -- & design variable; baseline 3~$\mu$m \\
Size span & -- & 1.2 & E & \\
...
Carbon inhibition scale & $C_{\mathrm{inh}}$ & 200~ppm & E & \\
...
Hydrogen inhibition constant & $K_{\mathrm{H}}$ & 25 & E & form after \cite{barrio2001steam} \\
...
Table~\ref{tab:ibm} summarises the result. The 785~$^\circ$C hold removes the char and leaves 47\% of the shrinkage to the burnout step, inside the 33--50\% reported for the process, while the pores stay open. At 830~$^\circ$C the body densifies completely during the hold and seals its pores. These two rows reproduce the calibration targets and are therefore not independent evidence. The 750~$^\circ$C row is: it depends on the gasification kinetics, which were not fitted, and it leaves four orders of magnitude more carbon than the 785~$^\circ$C hold. The model does not reproduce the absolute time scale of the patent's statement. With the estimated gasification rate, 33~ppm of carbon remain after 6~h at 750~$^\circ$C, which may or may not have been acceptable for IBM; no carbon specification was published. For all three holds the final body is 99\% dense and fully crystallised, and its free linear shrinkage is 23.1\%.
